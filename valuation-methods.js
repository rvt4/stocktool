...
function robustDecisionBlend(stock, category, lifecycle, primaryValuation, consensus, combined, legacyPriceTarget, model) {
  ...
  let expectedCAGR = currentPrice > 0 && actionableExitValue > 0 && years > 0
    ? Math.pow((actionableExitValue + dividends) / currentPrice, 1 / years) - 1
    : Number(primaryValuation?.expectedCAGR);

  // V41 consistency guardrail: the primary engine already reconciles valuation with
  // the projected operating economics. A weak legacy consensus is useful evidence,
  // but it should not re-introduce unlimited negative valuation drag after that
  // reconciliation. This is business-class logic, not a ticker override.
  const primaryExpected = Number(primaryValuation?.expectedCAGR);
  const operatingCAGR = Number(primaryValuation?.operatingCAGR);
  const primaryTrust = clamp(Number(primaryValuation?.valuationTrust ?? 0.5), 0, 1);
  const forwardGrowth = Number(lifecycle?.forwardGrowth ?? stock?.valuation?.businessForecast?.currentOperatingRate ?? 0);
  const economicallyStrongGrowth = growthCategory && forwardGrowth >= 0.18 &&
    Number.isFinite(operatingCAGR) && operatingCAGR >= 0.12;
  if (economicallyStrongGrowth && Number.isFinite(primaryExpected) && Number.isFinite(expectedCAGR)) {
    const maxConsensusPenalty = 0.025 + (1 - primaryTrust) * 0.025;
    expectedCAGR = Math.max(expectedCAGR, primaryExpected - maxConsensusPenalty);
  }

  // Keep the published exit value mathematically identical to the published CAGR.
  const finalActionableExitValue = currentPrice > 0 && Number.isFinite(expectedCAGR)
    ? Math.max(0, currentPrice * Math.pow(1 + expectedCAGR, years) - dividends)
    : actionableExitValue;

  return {
    fairValueToday, actionableExitValue: finalActionableExitValue, expectedCAGR, dividends, primaryWeight,
    ...
    version: 'v41-economics-aware-decision-blend',
  };
}
...
function valuateStock(stock, sectorExitMultiples, calibration = null) {
  ...
  const fiveYearPriceTarget = {
    ...
    returnEngineVersion: 'v41-economics-aware-category-and-valuation-rebuild',
    primaryValuation,
    decisionBlend,
  ...
  return {
    ...
    projectionAssumptions: {
      version: '41-canonical-economics-aware-decision-blend', category, lifecycle, moat, forecastHorizon: lifecycle.forecastYears, businessProfile, discountRate, terminalGrowth, analystReliability: analystReliability(stock), capitalAllocation: capitalAllocationScore(stock),
