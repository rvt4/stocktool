'use strict';
/** v12.55.6 Historical Price & Delisting Recovery Audit
...
async function main(){
 ...
 const factsCache=new Map(),historyCache=new Map(),recoveryCache=new Map();
 for(let i=0;i<work.length;i++){const {asOf,h}=work[i],c=coverage.get(asOf),aliases=aliasesFor(h);if(!aliases.length){add(c.failureReasons,'no_historical_ticker');continue;} const primary=aliases[0];
   let facts=factsCache.get(h.secCik);if(facts===undefined){try{facts=await fetchSecFactsByCik(h.secCik,primary);}catch{facts=null;}factsCache.set(h.secCik,facts);} if(!facts){add(c.failureReasons,'sec_facts_unavailable');continue;}
   ...
   if(diag.missingHistoricalPrice>0&&aliases.length>1){c.aliasRecoveryAttempts++;const alt=await historyForAliases(aliases.slice(1),historyCache);if(alt.history.length){let d2={usableFinancialHistory:0,insufficientFinancialHistory:0,historicalPriceFound:0,missingHistoricalPrice:0,shareCountFound:0,missingShareCount:0};let recYears=null;const rk=`${h.secCik}|${asOf}`;if(recoveryCache.has(rk))recYears=recoveryCache.get(rk)?.years||null;const s2=historicalStockFromData(primary,h.sector,facts,alt.history,asOf,d2,recYears);const m2=runModel(s2);if(m2.ok){c.aliasRecoverySuccesses++;c.aliasRecoveredModelable++;if(c.aliasExamples.length<10)c.aliasExamples.push({cik:h.secCik,asOf,primaryTicker:primary,recoveredPriceTicker:alt.ticker});}}}
   if(DELAY)await sleep(DELAY);
   if((i+1)%250===0||i===work.length-1)console.log(`Price/delisting audit ${i+1}/${work.length}`);
 }
 ...
 const report={generatedAt:new Date().toISOString(),version:'v12.55.6-historical-price-delisting-recovery-audit',requested:{startYear:START,endYear:END,frequency:FREQUENCY,requestedSnapshots:requested.length,auditedSnapshots:rows.length,unavailableSnapshots:unavailable,limit:LIMIT},guardrails:['Point-in-time IWB membership comes only from validated historical-universe caches.','Price alias retries are restricted to ticker aliases already attached to the same historical holding and SEC CIK.','No current-constituent membership fallback or unrelated successor-company substitution is permitted.','Historical valuation uses a quote on or before the snapshot; future prices cannot repair an earlier snapshot.','Legacy fundamentals recovery retains the v12.55.5 filing-date guardrails.','Null historical stocks are classified by production diagnostics so missing share count or fundamentals cannot be mislabeled as a price failure.'],summary:{holdingObservations:holdings,identityResolved:resolved,identityResolvedRate:pct(resolved,holdings),baselineModelable:base,recoveredModelable:rec,recoveredModelableRate:pct(rec,holdings),aliasRecoveredModelable:alias,aliasRecoveredModelableRate:pct(alias,holdings),incrementalAliasRecovered:alias-rec,failureReasons:failures},byYear:summarizeYear(rows),coverage:rows};
