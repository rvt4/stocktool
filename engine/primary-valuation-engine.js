...
/**
 * V58 economic-architecture primary valuation engine.
 *
...
function economicArchitecture(stock, category, lifecycle = {}) {
  const industry = stock?.valuation?.industryModel?.model || 'general';
  const stage = lifecycle.stage || category || 'Value';
  const years = (stock?.financials?.years || []).slice(-5);
  const last = years.at(-1) || {};
  const rev = Number(last.revenue || 0);
  const capex = Math.abs(Number(last.capex || last.capEx || 0));
  const fcf = Number(last.fcf || 0);
  const ni = Number(last.netIncome || 0);
  const positiveFcfHistory = years.length ? years.filter(y => Number(y.fcf) > 0).length / years.length : .5;
  const fcfConversion = ni > 0 ? clamp(fcf / ni, 0, 2) : null;
  const capitalIntensity = rev > 0 ? clamp(capex / rev, 0, .5) : null;
  ...
  let name = 'balanced-cash-flow';
  if (industry === 'financials') name = 'financial-earnings';
  else if (industry === 'reit') name = 'asset-income';
  else if (industry === 'utilities') name = 'regulated-cash-flow';
  else if (['energy','materials'].includes(industry) || capitalIntensity > .12 || ['Cyclical','Asset Heavy'].includes(stage)) name = 'cycle-normalized';
  else if ((industry === 'semiconductors-hardware' || industry === 'software' || forwardGrowth >= .18) && quality.persistence >= .50) name = 'innovation-growth';
  else if (positiveFcfHistory >= .8 && quality.quality >= .58) name = 'durable-owner-cash-flow';

  return { name, forwardGrowth, positiveFcfHistory, fcfConversion, capitalIntensity, quality };
}
...
function profileFor(stock, category, lifecycle = {}) {
  ...
  const architecture = economicArchitecture(stock, category, lifecycle);

  if (industry === 'financials') return {
    name: 'financial-earnings', primary: ['epsExit', 'dcf'], support: ['ownerEarnings'],
    weights: { epsExit: .60, ownerEarnings: .30, dcf: .10 }, maxBase: .18, maxRerating: .035,
  };
  ...
  if (industry === 'semiconductors-hardware') return {
    ...
    weights: growth >= .18 && persistence >= .55
      ? { epsExit: .40, dcf: .25, dcfSBCAdjusted: .20, revenueExit: .15 }
...
function reliability(method, presentValue, futureValue, centerPresent, centerFuture, profile, stock) {
  ...
  if ((profile.name === 'financial-earnings') && (method === 'dcf' || method === 'dcfSBCAdjusted')) r *= .30;
...
function selectedValuation({ stock, category, lifecycle, methodResults, model }) {
  ...
  const sortedValues = rows.map(x => x.presentValue).sort((a,b) => a-b);
  const valuationLow = sortedValues[Math.floor((sortedValues.length - 1) * .20)] || fairValueToday;
  const valuationHigh = sortedValues[Math.ceil((sortedValues.length - 1) * .80)] || fairValueToday;
  ...
  if (finite(adjustedCAGR)) {
    if (finite(operatingCAGR)) {
      ...
      // V58: an explicit competitive-fade bridge prevents a still-fast-growing business
      // from being treated as mature immediately at the investment horizon. The fade
      // allowance is earned by growth persistence/quality and is deliberately generic.
      const horizonGrowth = Number(exit.revGrowth ?? exit.revenueGrowth ?? forwardGrowth);
      const matureGrowth = clamp(Number(stock?.valuation?.terminalGrowth ?? .025), .015, .04);
      const fadeYears = clamp(Math.round(3 + quality.persistence * 5), 3, 8);
      const excessGrowth = Math.max(0, horizonGrowth - matureGrowth);
      const fadeSupport = excessGrowth * (.20 + .35 * quality.persistence) * (fadeYears / 5);
      if (horizonGrowth > .08 && finite(operatingCAGR)) {
        const fadeAnchor = Math.min(dynamicBaseCeiling, operatingCAGR) - clamp(.10 - fadeSupport, .025, .10);
        adjustedCAGR = Math.max(adjustedCAGR, fadeAnchor);
      }
  ...
  return {
    version: 'v58-economic-architecture-fade-valuation',
    ...
    fairValueToday,
    valuationRange: { low: valuationLow, central: fairValueToday, high: valuationHigh },
    valuationConfidence: agreementScore >= 70 ? 'High' : agreementScore >= 45 ? 'Medium' : 'Low',
    economicArchitecture: economicArchitecture(stock, category, lifecycle),
    rawExitValue,
    actionableExitValue,
    rawCAGR,
...
module.exports = { selectedValuation, profileFor, economicArchitecture };
