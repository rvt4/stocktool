...
function resolveInstitutionalValuationModel(stock, category, lifecycle = {}) {
  ...
  if (isFinancial) {
    ...
    return { key:'bank-earnings',label:'Bank earnings-power model',primary:['epsExit'],support:[],weights:{epsExit:1.00},maxBase:.16,maxRerating:.015,invalidMethods:['dcf','dcfSBCAdjusted','ownerEarnings','revenueExit','ebitdaExit'],useNetMarginForBridge:true,notes:['Conventional FCF, owner-earnings DCF and enterprise-value methods are disabled for deposit-funded balance sheets.','Bank fair value is anchored to normalized per-share earnings; a single-method result is explicitly treated as low-agreement evidence.'] };
  }
