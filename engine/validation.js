...
function validateStock(s){
  ...
  if(Number.isFinite(s.fairValueEstimate)&&Number.isFinite(s.marginOfSafety)&&p>0){const calc=s.fairValueEstimate/p-1;if(!near(calc,s.marginOfSafety))issues.push('mos_math_mismatch');}
  if(Number.isFinite(c)&&c>.25)issues.push('base_case_return_above_publication_ceiling');
