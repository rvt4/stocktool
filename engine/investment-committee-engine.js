...
function computeInvestmentCommitteeScore(stock,scenario,growthQuality,capital,competition){
 ...
 const quality=stock.valuation?.compounder?.score??stock.valuation?.businessProfile?.qualityScore??50; const fv=stock.valuation?.intrinsicValue??stock.valuation?.blendedFairValue??stock.valuation?.fairValueEstimate; const mos=fv>0?(fv-stock.price.current)/fv:0;
