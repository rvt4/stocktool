...
function strictDynamicMosProfileV1251(r){
  const p=dynamicMosProfile(r);
  if(p.tier==='Very high uncertainty')return {tier:'Insufficient confidence',requiredMOS:.30,eligibleQuality:false};
  return p;
}
...
function buildDynamicMosEntryLab(snapshotOutput){
  const strictV1251Eligible=r=>{const p=strictDynamicMosProfileV1251(r),mos=finite(r?.marginOfSafety);return p.eligibleQuality&&Number.isFinite(r?.expectedAlpha)&&r.expectedAlpha>=0&&Number.isFinite(r?.expectedCAGR)&&r.expectedCAGR>=.15&&Number.isFinite(r?.dynamicRank)&&r.dynamicRank<=25&&Number.isFinite(mos)&&mos>=p.requiredMOS&&String(r?.modelSupport||'')!=='unsupported'};
  const specs=[
    ...
    {key:'dynamic_mos_v1251',label:'B · v12.51 strict dynamic MOS',pick:rows=>dedupeEconomicSecurities(dynamicRankSnapshotRows(rows).filter(strictV1251Eligible).sort((a,b)=>a.dynamicRank-b.dynamicRank)).slice(0,15)},
    {key:'dynamic_mos_v1252',label:'C · v12.52 uncertainty-compensated MOS',pick:rows=>dedupeEconomicSecurities(dynamicRankSnapshotRows(rows).filter(ownerDynamicEntryEligible).sort((a,b)=>a.dynamicRank-b.dynamicRank)).slice(0,15)}
  ];
  const results=[];
  ...
  const a=results[0],c=results[2],replacements=[];if(a&&c){const am=new Map(a.cohorts.map(x=>[x.asOf,x])),cm=new Map(c.cohorts.map(x=>[x.asOf,x]));for(const [date,cc] of cm){const ac=am.get(date);if(!ac)continue;const aset=new Set(ac.tickers),cset=new Set(cc.tickers),added=cc.members.filter(x=>!aset.has(x.ticker)),removed=ac.members?.filter(x=>!cset.has(x.ticker))||[];replacements.push({asOf:date,added,removed});}}
...
function boundedNormalizedWeights(items,rawFn,minWeight=.05,maxWeight=.09){
 if(!items.length)return [];if(items.length*minWeight>1||items.length*maxWeight<1)return normalizeRawWeights(items,rawFn);
 const raw=items.map(x=>Math.max(.0001,rawFn(x))),w=Array(items.length).fill(null),open=new Set(items.map((_,i)=>i));let rem=1;
 for(let g=0;g<items.length+3&&open.size;g++){const rs=[...open].reduce((a,i)=>a+raw[i],0);let changed=false;for(const i of [...open]){const x=rem*raw[i]/rs;if(x<minWeight){w[i]=minWeight;rem-=minWeight;open.delete(i);changed=true}else if(x>maxWeight){w[i]=maxWeight;rem-=maxWeight;open.delete(i);changed=true}}if(!changed){const z=[...open].reduce((a,i)=>a+raw[i],0);for(const i of open)w[i]=rem*raw[i]/z;break}}
 const total=w.reduce((a,b)=>a+(b||0),0);return w.map(x=>(x||0)/total);
}
function v1254ConfidenceMultiplier(x){const a=[x.qualityScore,x.moatScore,x.compounderScore,x.forecastConfidence??x.forecastReliabilityScore??x.forecastConfidenceScore,x.valuationConfidence??x.valuationConfidenceScore??x.confidenceScore].map(finite).filter(Number.isFinite).map(v=>Math.max(0,Math.min(100,v))/100),c=a.length?mean(a):.5;return .90+.20*c}
function buildV1254WeightingLab(snapshotOutput){
 const specs=[{key:'equal',label:'A · Equal weight',weights:a=>a.map(()=>1/a.length)},{key:'alpha_mild',label:'B · Mild Alpha · current control',weights:a=>normalizeRawWeights(a,x=>alphaSizingTarget(finite(x.expectedAlpha)))},{key:'alpha_confidence_bounded',label:'C · Alpha + modest confidence · 5–9%',weights:a=>boundedNormalizedWeights(a,x=>alphaSizingTarget(finite(x.expectedAlpha))*v1254ConfidenceMultiplier(x),.05,.09)}],results=[];
 for(const spec of specs){const cohorts=[];for(const snap of snapshotOutput||[]){const picks=dedupeEconomicSecurities(dynamicRankSnapshotRows(snap.rows||[]).filter(ownerDynamicEntryEligible).sort((a,b)=>a.dynamicRank-b.dynamicRank)).slice(0,15),items=picks.filter(x=>Number.isFinite(x.realized5YTotalReturnCAGR)&&Number.isFinite(x.spy5YTotalReturnCAGR));if(!items.length)continue;const weights=spec.weights(items),portfolioCAGR=weightedHoldCAGR(items,weights,5),spyCAGR=cohortSpyCAGR(items,5),tv=items.map((x,i)=>weights[i]*Math.pow(Math.max(0,1+x.realized5YTotalReturnCAGR),5)),tt=tv.reduce((a,b)=>a+b,0),tw=tv.map(x=>x/tt);cohorts.push({asOf:snap.asOf,holdings:items.length,portfolioCAGR,spyCAGR,excessCAGR:portfolioCAGR-spyCAGR,beatSpy:portfolioCAGR>spyCAGR,hit15:portfolioCAGR>=.15,initialMinWeight:Math.min(...weights),initialMaxWeight:Math.max(...weights),terminalMaxWeight:Math.max(...tw),removeBest1CAGR:weightingWinnerRemoval(items,weights,5,1),removeBest3CAGR:weightingWinnerRemoval(items,weights,5,3),allocations:items.map((x,i)=>({ticker:x.ticker,rank:x.dynamicRank,expectedAlpha:x.expectedAlpha,qualityScore:x.qualityScore,forecastConfidence:x.forecastConfidence??x.forecastReliabilityScore??x.forecastConfidenceScore,valuationConfidence:x.valuationConfidence??x.valuationConfidenceScore??x.confidenceScore,initialWeight:weights[i],realized5YCAGR:x.realized5YTotalReturnCAGR,terminalWeight:tw[i]}))})}results.push({key:spec.key,label:spec.label,cohortCount:cohorts.length,meanHoldings:mean(cohorts.map(x=>x.holdings)),meanPortfolioCAGR:mean(cohorts.map(x=>x.portfolioCAGR)),medianPortfolioCAGR:median(cohorts.map(x=>x.portfolioCAGR)),meanSpyCAGR:mean(cohorts.map(x=>x.spyCAGR)),meanExcessCAGR:mean(cohorts.map(x=>x.excessCAGR)),beatSpyRate:cohorts.length?cohorts.filter(x=>x.beatSpy).length/cohorts.length:null,hit15Rate:cohorts.length?cohorts.filter(x=>x.hit15).length/cohorts.length:null,worstCohort:cohorts.reduce((a,b)=>!a||b.portfolioCAGR<a.portfolioCAGR?b:a,null),meanInitialMinWeight:mean(cohorts.map(x=>x.initialMinWeight)),meanInitialMaxWeight:mean(cohorts.map(x=>x.initialMaxWeight)),meanTerminalMaxWeight:mean(cohorts.map(x=>x.terminalMaxWeight)),meanRemoveBest1CAGR:mean(cohorts.map(x=>x.removeBest1CAGR)),meanRemoveBest3CAGR:mean(cohorts.map(x=>x.removeBest3CAGR)),cohorts})}
 return {description:'v12.54 weighting-only challenger. Selection is frozen to the exact v12.52 uncertainty-compensated Top-15 entry policy. Equal weight and Mild Alpha are controls. The challenger keeps Alpha primary, adds only a 0.90–1.10 quality/confidence multiplier, and constrains initial positions to 5–9% when feasible. MOS and Valuation Rating are not additional sizing multipliers. No sell changes.',results};
}
...
function buildLongTermOwnerLab(rows,snapshotOutput,historyByTicker=null,spyHistory=null){
  ...
  return {
    description:'v12.52 long-term-owner test. Entry requires >=15% expected CAGR, dynamic Model-D rank <=25, supported valuation, and a quality/confidence-scaled margin of safety. Elite established compounders need 5% MOS, strong established businesses 10%, standard quality 20%, higher-uncertainty names 25%, and very-high-uncertainty names 35%. Uncertainty demands more compensation rather than becoming an automatic veto. Outcomes are measured from the original purchase signal with no rank-based selling.',
