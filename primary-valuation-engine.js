...
function selectedValuation({ stock, category, lifecycle, methodResults, model }) {
  ...
  // Financial companies and other specialized businesses can legitimately have only
  // one economically applicable method with complete data. Do not throw that method
  // away and fall back to a generic legacy valuation merely because a second method
  // is unavailable. General businesses still require corroboration.
  const minMethods = profile.name === 'financial-earnings' ? 1 : 2;
  if (all.length < minMethods) return null;
  ...
  return {
    version: 'v41-economics-aware-primary-valuation',
